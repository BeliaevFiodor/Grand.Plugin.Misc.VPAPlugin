...
using Grand.Web.Features.Handlers.Catalog;
...
{
    public class GetCategoryHandlerOver : GetCategoryHandler, IRequestHandler<GetCategory, Web.Models.Catalog.CategoryModel>
    {
        ...
        public GetCategoryHandlerOver(
            ...
            MediaSettings mediaSettings) : base(mediator, webHelper, priceFormatter, currencyService, cacheManager, categoryService, pictureService, localizationService, specificationAttributeService, httpContextAccessor, catalogSettings, mediaSettings)
        {
            _mediator = mediator;
        ...
        public new async Task<Web.Models.Catalog.CategoryModel> Handle(GetCategory request, CancellationToken cancellationToken)
        {
            ...
            if (_catalogSettings.CategoryBreadcrumbEnabled)
            {
                ...
                model.CategoryBreadcrumb = await _cacheManager.GetAsync(breadcrumbCacheKey, async () =>
                    ...
                    .Select(catBr => new Web.Models.Catalog.CategoryModel {
                        Id = catBr.Id,
            ...
            //subcategories
            var subCategories = new List<Web.Models.Catalog.CategoryModel.SubCategoryModel>();
            foreach (var x in (await _categoryService.GetAllCategoriesByParentCategoryId(request.Category.Id)).Where(x => !x.HideOnCatalog))
            {
                var subCatModel = new Web.Models.Catalog.CategoryModel.SubCategoryModel {
                    Id = x.Id,
